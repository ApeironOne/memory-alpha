...
export function registerHooks(ctx: OpenClawPluginContext, config: MemoryAlphaConfig) {
  if (config.autoCapture) {
    ctx.hooks.on("message:received", async (_ctx) => {
      // TODO: extract memories from inbound messages (LLM extraction)
      // placeholder: no-op
    });

    ctx.hooks.on("message:sent", async (_ctx) => {
      // TODO: extract memories from outbound messages (LLM extraction)
      // placeholder: no-op
    });
  }
  ...
  if (config.autoRecall) {
    ctx.hooks.on("before_prompt_build", async (_ctx) => {
      // TODO: run recall and inject context into prompt
      // placeholder: no-op
    });
  }
